...
test.describe("Notty App Note Tests", () => {
  test("should create a new note", async ({ page }) => {
    ...
    await page
      ...
      .locator("div")
      .filter({ hasText: "Start writing here...." })
      .fill("This is a test note");
    await page.getByRole("button", { name: "Create Note" }).click();
    await expect(
      page.getByText("New Note created successfully!"),
    ).toBeVisible();
  });
  ...
  test("should view note details", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "View Note" }),
    ).toBeVisible();
    await page
  ...
  test("update disable", async ({ page }) => {
    ...
    await page
      .locator("div")
      .filter({
        hasText: /^This is a test note for update test hjkdfjkjdEdit$/,
      })
      .getByTestId("edit-button")
      .click();
    await expect(page.getByText("CancelUpdate Note")).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Update Note" }),
    ).toBeDisabled();

    await page
      .getByTestId("quill-editor")
      .getByText("This is a test note for")
      .click();
    await page
      .getByTestId("quill-editor")
      .locator("div")
      .filter({ hasText: "This is a test note for" })
      .fill("This is a test note for update test hjkdfjkjd , new data");
    await page.getByRole("button", { name: "Update Note" }).click();
